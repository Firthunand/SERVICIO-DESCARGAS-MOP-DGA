#!/bin/bash
# Script de arranque del servicio de descargas MOP DGA
# Ejecutar desde la raíz del proyecto: ./scripts/start_servicio.sh
...
# 4. Flask (entorno virtual si existe)
echo ""
echo "Iniciando aplicación Flask en $PROYECTO_ROOT"
if [ -d "$PROYECTO_ROOT/env" ]; then
  . "$PROYECTO_ROOT/env/bin/activate"
fi
# Sin exec para que Ctrl+C dispare cleanup y cierre Xvfb/Openbox/x11vnc
